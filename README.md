...
在进行了完全冗余和部分冗余之后，会删除或者增加一些block，因此需要重新构造合约的字节码。这是十分头疼的一个问题：

* 在进行冗余优化的时候，因为是一次性搜索出所有的路径，因此路径的信息也是按照原cfg中的block进行记录的。所以说，为了不影响不同路径直接的优化，每次完成一个invalid的优化后，不能将新block、新边信息直接在cfg中进行修改，只能是等到所有的路径都得到处理之后，才能统一填入。这个过程中，如何记录原有的信息，记录哪些信息便成为难题

#### 解决办法

在每次做一处优化之后，要利用function类检查函数的调用边/返回边是否涉及了这些改变，如果涉及了，则要将其修改为新的调用边/返回边，从而方便后来进行重定位。

### 重定位

一些待验证的假设：

* 所有的函数返回地址都是在调用边的入点出push
* jumpi都是这样的格式：`计算跳转条件;push 跳转地址;jumpi`
