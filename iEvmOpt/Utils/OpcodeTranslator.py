class OpcodeTranslator:
    ...
    def __init__(self, blocks: dict,exitNode:int):
        self.blocks = blocks
    ...
    def translate(self):
        ...
        for offset in self.blocks.keys():
            if offset == self.exitNode:
                self.blocks[offset].instrs = ["{}:EXIT BLOCK".format(self.blocks[offset].offset)]
                continue
            addrToContent = {}  # 地址：内容
            bytecode = self.blocks[offset].bytecode
            blockLen = self.blocks[offset].length
            PC = 0
            while PC < blockLen:
                tempPc = PC
                opcode = bytecode[PC]
                match opcode:
                    ...
                    case i if 0x90 <= opcode <= 0x9f:  # swap
                        depth = opcode - 0x90 + 1
                        addrToContent[tempPc] = "DUP" + str(depth)
                    case 0xf3:
                    ...
                    case _:  # Pattern not attempted
                        ...
                        assert 0, err
                PC += 1
            contentList = ["{}:{}".format(self.blocks[offset].offset + addr, c) for addr, c in addrToContent.items()]
            self.blocks[offset].instrs = contentList

    def getParsedOpcode(self, node: int):
        return self.blocks[node].instrs
